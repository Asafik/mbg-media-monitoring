...
/** Animates a numeric string counting up from 0 to target */
function useCountUp(target: string, durationMs = 1200) {
...
function getIcon(type: KpiItem['type']) {
  switch (type) {
    case 'content':
      return (
...
/** Single KPI card with count-up number animation */
const KpiCard: React.FC<{ item: KpiItem }> = ({ item }) => {
  const animatedValue = useCountUp(item.value)
  const isUp = item.isPositiveChange
  return (
    <div className="bg-white rounded-lg p-4 border border-slate-200 shadow-xs flex flex-col justify-between hover:shadow-sm transition-shadow">
